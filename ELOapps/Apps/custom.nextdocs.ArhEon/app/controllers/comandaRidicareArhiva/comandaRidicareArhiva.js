        ...
        function($scope, $filter, ModalDialogService, FileService, $timeout, AuditService, $rootScope, ArhRidicareService) {
            ...
            ctrl.adaugaComanda = function() {
                ...
                if (!ctrl.comanda.tip_arhiva ||
                    ...
                    !ctrl.comanda.crc ||
                    !ctrl.comanda.persoana ||
                    !ctrl.comanda.telefon) {
                    return;
                }
            ...
            ctrl.inregistreazaComenzi = function() {
                ...
                if (res) {
					var data_inregistrare = $filter('date')(new Date(), 'yyyy-MM-dd HH:mm:ss');
					var payload = angular.copy(ctrl.comenzi);
					
					payload.forEach(comanda => {
						comanda.data_inregistrare = data_inregistrare
					});

					ArhRidicareService.createRidicareArhiva(payload).then(function (res) {

                        var numbers = (res && Array.isArray(res.numere_generate))
                            ? res.numere_generate
                            : [];
                    
                        if (!numbers.length) {
                            ModalDialogService.showModalDialog({
                                title: "Arhivare - Eroare solicitare ridicare",
                                noHeader: false,
                                template: `
                                    <p>Nicio solicitare nu a fost inregistrata!</p>
                                `,
                                buttons: [
                                    {
                                        name: "ok",
                        ...
                        var dateText = (res && res.date)
                            ? res.date.replaceAll("'", "")
                            : "-";
                    
                        ModalDialogService.showModalDialog({
                            title: "Arhivare - Solicitare ridicare",
                            noHeader: false,
                            template: `
                                <p>Solicitarea a fost inregistrata!</p>
                                <p>${label}<br>${numbersHtml}</p>
                                <p>Data inregistrarii: ${dateText}</p>
                            `,
                            buttons: [
                                {
                                    name: "ok",
                        ...
                        console.error("Eroare la inregistrarea solicitarilor de ridicare:", err);
            ...
            ctrl.$onInit = function() {
                ...
                ctrl.searchNomenclator("[Lucru_EON].[dbo].[arh_departament_ridicare]", "optiuniDepartament");
